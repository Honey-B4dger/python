...
#        print (self.canvas.get_tk_widget())
    def plotxy(self, x, y):        
        self.axes.scatter(x,y)  # Scatter Plotfunktion aufrufen    
        self.canvas.draw()      # Fenster aufbauen (= zeichnen)
    def clearplot(self):
...
class ReadData():
    ...
    def myscat(self):
        filename = '../data/eq_data_1_day_m1.json' # Dateiname muß angepasst werden an lokalen Speicherort, sonst Fehler. 
...
def plotdata():
    b2.invoke()              # Button b2 "ausführen" (clear)
    x,y = datrd.myscat()     # Funktion ausführen
    plot_w.plotxy(x,y)

def clear():    
    plot_w.clearplot()      # Fensterinhalt löschen

if __name__ == "__main__":  # verhindert Start bei Import; ermöglicht Start bei Ausführng als Executable. 
    
    datrd = ReadData()                                        # Hauptprogramm
    root = Tk()
    root.title("MyPlot")
    mainframe = Frame()
    plot_w = Plotwindow(mainframe,(8,6))                     # Plotfenster aufbauen
    mainframe.grid(row = 0,rowspan = 8,column = 1)
    buttonframe = Frame()                                    # Buttonleiste anlegen
