...
scpl = ax.scatter(lons, lats, marker="o", color="C1", s = smags, alpha=1.0,label="eqs magnitude", edgecolors = "k",zorder = 1)  # color= RGBA-Wert 
                                                 # edgecolors = "k" liefert schwarzen Rand um Marker
for i,txt in enumerate(mags): 
#    print (i,txt,lons[i])
    ax.annotate(txt,xy=(lons[i],lats[i]),xytext=(2, 3),  # 2 points horizontal, 3 points vertical offset
    textcoords="offset points",ha="left",va="bottom",clip_on=True, zorder = 2, fontsize =10)
                                                 # myplt.annotate( label,  
                                                 #                 (x,y),
                                                 # textcoords = "offset points", (keyword for offset coordinate system)
...
#mng.window.showMaximized()                       # Maximize Window
